...
function connectToClient() {
    if (!connected)
    {
        if (window.location.href.search("localhost") > -1) {
            socket = new WebSocket("ws://localhost:45555/");
        }
        else {
            socket = new WebSocket("ws://65.182.241.7:45555/");
        };
        ...
        socket.onmessage = function (e) {
            ...
            switch (jsonMessage.Type) {
                case "Connect":
                    ...
                    {
                        // Skip RTC since it requires TLS.
                        //initRTC();
                        var request = {
                            "Type": "CaptureScreen",
                            "Source": "WebSocket",
                    ...
                    break;
                case "Capture":
                    // Incoming data is a screen capture in data URI format.
                    if ($("#canvasScreenViewer").is(":hidden"))
                    {
                        $("#canvasScreenViewer").show();
                    }
                    // Data URI is decoded, converted to Blob, then read as ArrayBuffer.
                    fr.onload = function(){
                        var byteArray = new Uint8Array(fr.result);
                        var length = byteArray.length;
                        // Retrieve the appended bytes that tells where this image should be
                        // drawn on the canvas.  This is necessary because I'm sending a bounding
                        // box of only the pixels that changed on this frame, so the image is hardly
                        // ever placed at x 0, y 0 coordinates.  The XY coordinates were appended to
                        // to the bitmap data by the client app.
                        imgX = Number(byteArray[length - 4] * 100 + byteArray[length - 3]);
                        imgY = Number(byteArray[length - 2] * 100 + byteArray[length - 1]);
                        // Get an object URL for only the actual bitmap data.
                        url = window.URL.createObjectURL(new Blob([byteArray.subarray(0, length - 4)]));
                        img = document.createElement("img");
                        img.onload = function () {
                            // Load the data into an img element and draw it on the canvas.
                            context.drawImage(img, imgX, imgY, img.width, img.height);
                            window.URL.revokeObjectURL(url);
                        };
                        img.src = url;
                    }
                    fr.readAsArrayBuffer(dataURItoBlob(atob(jsonMessage.DataURI)));
                    break;
                case "Bounds":
